...
def load_data(directory, debug):

    # data_list = []

    # load data
    current_file = directory + 't' + str(config.TOTAL_ROBOTS) + '_r' + str(config.MAX_RUNNING_ROBOTS) + '_d' + str(config.NUM_DATA_POINTS) + '.pkl'
    print('Load data: ', current_file)
    generate_data_seed, data = pickle.load(open(current_file, 'rb'))
    # data_list += data

    # raw_data = np.array(data_list)
    raw_data = np.array(data[:config.NUM_DATA_POINTS_USED])
    if debug:
          print(raw_data[:10])
...
if __name__ == "__main__":
    ...
    genomes, scaler = load_data(DATA_DIRECTORY, debug)
    ...
    for i in range(BEST_OUT_OF):
        ...
        if debug: 
            print('Run', i)
        vae = VecVAE(n_dim, config.NUM_LATENT)

        # Train VAE
        vae, loss = train_vae(genomes, vae, config.NUM_EPOCHS, view_mod=2)
    ...
    if debug:
        ...
        n_ind = 1
        random_genome = np.random.randn(n_ind, config.NUM_LATENT)
        ...
        not_so_random_genome = np.zeros([n_ind, config.NUM_LATENT])
